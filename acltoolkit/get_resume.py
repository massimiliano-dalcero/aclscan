...
import acltoolkit.rev_resume as rs
...
class GetResumeAcl:
    ...
    def run(self):
        infos = None
        outfile = None
        ignore_sids = set(self.options.ignore_sid or [])
        ignore_name_patterns = list(self.options.ignore_name or [])

        if self.options.out is not None:
            outfile = self.options.out

        if self.options.input:
            with open(self.options.input, "r", encoding="utf-8") as f:
                infos = json.load(f)
        else:
            self.connect()
            ...
            for user in self.users + self.groupss:
                self.object = user
                ...
                infos.append(object_info)

        rev = rs.rev(infos, ignore_sids, ignore_name_patterns, self.options.no_default_ignores)

        if outfile:
            rev_outfile = f"rev_{outfile}"
            with open(outfile, "w", encoding="utf-8") as f:
                json.dump(infos, f, indent=2, ensure_ascii=False)

            with open(rev_outfile, "w", encoding="utf-8") as f:
                json.dump(rev, f, indent=2, ensure_ascii=False)

            print(f"[+] JSON successfully saved to {outfile}")
            print(f"[+] JSON successfully saved to {rev_outfile}")

        #else:
        #    print(json.dumps(infos, indent=2))
        #pretty_print(object_info)

        if not outfile or self.options.print_human:
            #print("Normal:")
            #for info in infos:
            #    pretty_print(info)
            #print("#"*80)
    ...
    @property
    def groupss(self) -> LDAPEntry:
        if self._groupss is not None:
            return self._groupss
        ...
        self._groupss = objects

        return self._groupss


    @property
    def xobject(self) -> LDAPEntry:
...
def get_resume(options: argparse.Namespace):
    g = GetResumeAcl(options)
    g.run()
